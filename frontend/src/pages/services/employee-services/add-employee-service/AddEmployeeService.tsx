...
interface AddEmployeeServiceProps {
  isOpen: boolean;
  onAddClick: (employeeService: EmployeeService) => void;
  onCloseClick: () => void;
}

const initialState: EmployeeService = {
  id: 0,
  service_id: 0,
  start_time: "",
  end_time: "",
...
const AddEmployeeService: React.FC<AddEmployeeServiceProps> = ({ isOpen, onAddClick, onCloseClick }) => {
  const dispatch: AppDispatch = useDispatch();
  
  const { services, error } = useSelector((state: RootState) => state.data.services.data);
  
  const [values, setValues] = useState<EmployeeService>(initialState);
  ...
  return (
    <div className={`add-employee-service ${isOpen ? "active" : ""}`}>
      <div className="add-employee-service__container">
        <div className="add-employee-service__wrapper">
          <div className="add-employee-service__header">
            <div className="add-employee-service__title">Додавання нової послуги</div>
            <div>
              <div className="add-employee-service__close" onClick={handleClose}></div>
            </div>
          </div>

          <div className="add-employee-service__content">
            {services.length > 0 && (
          ...
          <div className="add-employee-service__footer">
            <Button 
              text="Додати" 
              type="button" 
