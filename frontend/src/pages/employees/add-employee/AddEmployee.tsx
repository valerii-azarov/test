...
const AddEmployee: React.FC<AddEmployeeProps> = ({ isOpen, onAddClick, onCloseClick }) => {
  ...
  const { error } = useSelector((state: RootState) => state.data.employees.actions.addEmployee);

  const [values, setValues] = useState<Employee>(initialState);
  ...
  const validate = useCallback((data: Employee) => {
    return Object.entries(data).reduce((acc: Record<string, string>, [key, value]) => {
      ...
      if (typeof value === "string") {
        const stringFields = ["phone", "email", "username", "password", "confirmPassword", "position_id", "region_id", "city_id", "office_id"];
  ...
  return (
    <div className={`add-employee ${isOpen ? "active" : ""}`}>
      <div className="add-employee__container">
        <div className="add-employee__wrapper">
          <div className="add-employee__header">
            <div className="add-employee__title">Додавання нового співробітника</div>
            <div>
              <div className="add-employee__close" onClick={handleClose}></div>
            </div>
          </div>
          ...
          <div className="add-employee__footer">
            <Button text="Додати" type="button" onClick={handleAddEmployee} />
          </div>
        </div>
      </div>
